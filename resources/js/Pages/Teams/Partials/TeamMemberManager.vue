            ...
            <jet-form-section @submitted="addTeamMember">
                ...
                <template #form>
                    ...
                    <div class="col-span-6 lg:col-span-4" v-if="availableRoles.length > 0">
                        ...
                        <div class="relative z-0 mt-1 border border-slate-200 dark:border-slate-600 rounded-lg cursor-pointer">
                            <button type="button" class="relative px-4 py-3 inline-flex w-full rounded-lg focus:z-10 focus:outline-none focus:border-blue-300 dark:focus:border-blue-600 focus:ring focus:ring-blue-200 dark:focus:ring-blue-600"
                                            :class="{'border-t border-slate-200 dark:border-slate-600 rounded-t-none': i > 0, 'rounded-b-none': i != Object.keys(availableRoles).length - 1}"
                                ...
                                <div :class="{'opacity-50': addTeamMemberForm.role && addTeamMemberForm.role != role.key}">
                                    ...
                                    <!-- Role Description -->
                                    <div class="mt-2 text-xs text-slate-600 dark:text-slate-300 text-left">
                                        {{ role.description }}
                                    </div>
                                </div>
                            </button>
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center justify-between" v-for="user in team.users" :key="user.id">
                            ...
                            <div class="flex items-center">
                                ...
                                <button class="ml-2 text-sm text-slate-400 underline"
                                        @click="manageRole(user)"
                                        v-if="userPermissions.canAddTeamMembers && availableRoles.length">
                                    {{ displayableRole(user.membership.role) }}
                                </button>

                                <div class="ml-2 text-sm text-slate-400" v-else-if="availableRoles.length">
                                    {{ displayableRole(user.membership.role) }}
                                </div>
                ...
                <div v-if="managingRoleFor">
                    <div class="relative z-0 mt-1 border border-slate-200 rounded-lg cursor-pointer">
                        <button type="button" class="relative px-4 py-3 inline-flex w-full rounded-lg focus:z-10 focus:outline-none focus:border-blue-300 focus:ring focus:ring-blue-200"
                                        :class="{'border-t border-slate-200 rounded-t-none': i > 0, 'rounded-b-none': i !== Object.keys(availableRoles).length - 1}"
